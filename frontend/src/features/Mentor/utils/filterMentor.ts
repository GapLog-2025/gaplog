...
export function filterMentorsByTags(
  reviews: Mentor[],
  selectedTags: string[],
): Mentor[] {
  const categoryTags = [
...
export function matchReviewByKeyword(review: Mentor, keyword: string): boolean {
...
export function matchMentorByKeyword(review: Mentor, keyword: string): boolean {
  const lowered = keyword.toLowerCase();
